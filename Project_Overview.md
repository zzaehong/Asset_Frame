...
| ETF·주요지수·13F 기반 유니버스 v2 정책 | 문서 확정, 구현 예정 |
...
| Universe | ETF 카탈로그·SEC 13F·KRX 지수 구성에 따른 편입 근거와 유효기간 관리 | 중복 노출 제거·투자 추천 |
...
| 리스크 우선, 종합점수 없음 | 허위 정밀도와 판단 위임 방지 | PRD 4장 |
| 숫자와 AI 책임 분리 | 재현성·감사 가능성·환각 통제 | PRD 4장 |
...
| 근거 기반 ETF·주식 유니버스 | 공급자 한도 안에서 분석 가치와 재현 가능한 편입 근거 확보 | ADR-0003 |

## 11. 다음 구현 순서

1. 한미 ETF 합산 120개 카탈로그와 공통 노출 분류를 만들되 동일 노출의 국가별 상품을 모두 허용
2. KRX 공식 KOSPI 200·KOSDAQ 150 구성종목 snapshot과 편입·편출 이력 수집
3. SEC 13F 기반 미국 투자자 관심 주식 최대 200개 선정과 근거 이력 구현
4. Tiingo 월간 고유 심볼 400개 예산을 적용하고 기존 전체 종목 discovery 경로를 교체
5. 선정 유니버스의 최근 10년 가격·기업행동 backfill과 coverage 검증
6. 주요 정기·사건 공시, 재무사실, ETF 보유내역과 한미 거시 시계열 수집
7. 최근 7일 뉴스 메타데이터의 제한된 backfill과 일 단위 증분 수집
8. Data Console에 유니버스 근거·coverage·quarantine·공급원 비교 화면 추가
9. 투자자산운용사 목차를 분석 도메인·공식·사용자 설명으로 매핑
10. 기본·기술·ETF·포트폴리오 엔진과 공식 공시 우선 AI 문서 분석 구현
