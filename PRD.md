...
분석 대상은 편입 근거가 명시된 다음 유니버스로 관리한다.

- 한국·미국 상장 ETF 합산 최대 120개. 국가·거래소·ticker가 다른 상품은 같은 지수나
  자산군을 추종하더라도 별도 자산으로 허용한다. 공통 `exposure_key`는 비교·중복 노출
  분석에만 사용하며 편입 제외 조건으로 사용하지 않는다.
- 미국 개별주식 최대 200개. 설정에 등록한 유명 투자자·운용사의 SEC 13F에서 상위 보유,
  신규 편입, 의미 있는 보유 확대가 확인된 종목만 관리한다. 13F는 공개 보유 사실이지
  실시간 관심이나 투자 추천으로 해석하지 않는다.
- 한국 개별주식은 KRX 공식 KOSPI 200·KOSDAQ 150 구성종목을 기준으로 관리한다. 지수
  구성 스냅샷, 적용일과 편입·편출 이력을 보존한다.

Tiingo를 호출하는 미국 ETF와 미국 관심 주식은 한 달에 사용한 고유 심볼 400개를 넘지
않는다. 해당 월에 편출되거나 검증·신규 backfill에 사용된 심볼도 한도에 포함한다. KRX로
수집하는 한국 ETF와 한국 주식은 Tiingo 예산에 포함하지 않는다. 모든 유니버스 선정은
입력·기준일·근거 snapshot·규칙 버전을 보존하며 투자 매력도 점수로 사용하지 않는다.

### 7.3 Phase 2 — AI Document Analysis

- 정기 재무보고와 주요 사건 공시, IR, API로 발견한 최근 뉴스 메타데이터의 문서 유형·기업·사건·시점 추출
- 주장과 근거, 영향 경로, 기간, 반대 논리, 불확실성 구조화
- 공식 공시의 변경점 비교
- 근거 URL·근거 구간·모델·프롬프트 버전 기록

뉴스는 장기 원문 저장소를 목표로 하지 않는다. 초기에는 최근 7일 이내의 제목·API 제공
요약·매체·발행시각·원문 URL만 보존하고 기사 본문을 자동 수집하지 않는다. 공시는 모든
유형을 무차별 수집하지 않고 정기 재무보고와 사전에 정의한 주요 사건 form만 canonical
분석 대상으로 삼으며, 나머지는 필요할 때 원문 링크로 확인한다.
...
| 미국 투자자 보유 | SEC EDGAR 13F | WhaleWisdom 공식 API(선택) | SEC 13F 원문 링크 |
...
| DATA-009 | 근거 기반 유니버스 | ETF 카탈로그, SEC 13F, KOSPI 200·KOSDAQ 150 중 하나 이상의 편입 근거와 유효기간을 보존한다. |
| DATA-010 | 공급자 심볼 예산 | Tiingo 월간 고유 심볼을 400개 이하로 유지하고 이월·거절 사유를 작업 상태에 기록한다. |
...
- 저장량: raw store와 PostgreSQL 사용량을 관측하고 경고 기준을 설정하되 고정 hard limit로 데이터 수집을 임의 중단하지 않음
...
| 13F 공개 지연·불완전성 | 기준분기·제출일·최대 45일 지연 가능성과 비공개·비대상 포지션 한계를 표시 |
| ETF 노출 중복 오해 | 동일 노출의 한미 상품을 별도 자산으로 유지하고 국가·통화·거래시간·비용 차이를 함께 표시 |
