...
GDELT는 API 키가 필요하지 않으며 DOC 2.0 API로 최근 해외 뉴스 메타데이터를 수집합니다.
한 요청의 기간은 최대 90일, 결과는 최대 250건으로 제한하고 기사 본문은 저장하지 않습니다.
공용 API의 HTTP 429 제한이나 일시 장애가 발생하면 해당 수집 실행을 실패로 기록한 뒤 나중에
재시도해야 합니다.
